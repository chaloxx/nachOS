...
Scheduler::Scheduler()
{
  queue = new List<Thread *>* [L_PRIORITY];
  for(int i = 0; i < L_PRIORITY;i++){
    DEBUG('g',"Inicializando cola con prioridad %d",i);
    queue[i] = new List<Thread*>;
  }
}
...
Scheduler::ReadyToRun(Thread *thread,int priority)
{
    ...
    DEBUG('g',"Poniendo %s en la cola de listos con prioridad %d\n", thread->GetName(),priority);
    thread->SetStatus(READY);
    //Agregar hilo a la cola de la prioridad que le corresponda
    queue[priority]->Append(thread);
}
...
{
  DEBUG('g',"Encontrando proximo a lanzar\n");
  //El próximo a lanzar será el que esté en frente de la cola de mayor prioridad
  for (int i = L_PRIORITY-1; i >= 0; i--) {
    if(!queue[i]->IsEmpty()){
      DEBUG('j',"2\n");
      Thread* next =  queue[i]->Pop();
      DEBUG('g',"El proximo a lanzar será %s\n",next -> GetName());
      return next;   
    }      
  }
  return nullptr;// Todas las colas estan vacías
}
