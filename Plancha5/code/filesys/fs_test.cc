...
Copy(const char *from, const char *to)
{
    ...
    if (!fileSystem->Create(to)) {  // Create Nachos file.
    ...
    int amountRead;   
    DEBUG('f',"Escribiendo en %s\n",to);

    while ((amountRead = fread(buffer, sizeof(char),
                               TRANSFER_SIZE, fp)) > 0)
            openFile->Write(buffer, amountRead);
    DEBUG('f',"Ya termine de escribir\n");
    delete [] buffer;
...
FileWrite()
{
    ...
    DEBUG('f',"Voy a escribir y content size: %u\n",CONTENT_SIZE);
