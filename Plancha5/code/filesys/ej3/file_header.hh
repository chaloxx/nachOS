...
const unsigned META_DATA_SIZE = SECTOR_SIZE / sizeof(unsigned);//Cantidad de unsigned que voy a pode
const unsigned MAX_FILE_SIZE = NUM_DIRECT * META_DATA_SIZE * SECTOR_SIZE;
...
public:
    ...
    RawFileHeader *GetRaw() const;

    //Retorna la cantidad de sectores ocupados por el archivo
    unsigned GetNumSectors() const;

    //Calcular cuantos bytes falta allocar
    unsigned MissingBytes(unsigned position,unsigned bytes);

private:
    unsigned numBytes = 0; // Tamaño en bytes del archivo
    unsigned metaDataSectors[NUM_DIRECT]; // Mapa a sectores de metadatos
    void CompleteLastMetaDataSector(unsigned,unsigned,unsigned*,Bitmap*);
};
