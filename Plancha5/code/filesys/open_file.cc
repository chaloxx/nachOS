...
OpenFile::OpenFile(unsigned s)
{
    ...
    hdr->FetchFrom(sec);
    seekPosition = 0;
    DEBUG('f',"El archivo tiene tamaño %u\n",hdr->FileLength());
}
...
OpenFile::Read(char *into, unsigned numBytes)
{
    ...
    ASSERT(into != nullptr);
    ASSERT(numBytes > 0);    
    LockWR *fileLock = openFileTable -> Get(sec);
    ASSERT(fileLock != nullptr);
    fileLock -> ReaderLock();//Bloquear el acceso a escritores*/
    int result = ReadAt(into, numBytes, seekPosition);
    fileLock -> ReaderUnlock();
    seekPosition += result;
    return result;
}
...
OpenFile::Write(const char *into, unsigned numBytes)
{
    ...
    ASSERT(into != nullptr);
    ASSERT(numBytes > 0);
    DEBUG('f',"Se me pide escribir %u bytes\n",numBytes);
    LockWR *fileLock = openFileTable -> Get(sec); 
    ASSERT(fileLock != nullptr);
    fileLock -> WriterLock();//Bloquear el acceso a lectores y escritores*/
    int result = WriteAt(into, numBytes, seekPosition);
    fileLock -> WriterUnlock();
    seekPosition += result;
    return result;
}
...
OpenFile::ReadAt(char *into, unsigned numBytes, unsigned position)
{
    ...
    char *buf;
    DEBUG('f',"el length: %u y la pos %u\n",hdr->FileLength(),position);
...
OpenFile::WriteAt(const char *from, unsigned numBytes, unsigned position)
{
    ASSERT(from != nullptr);
    ASSERT(numBytes > 0);
    unsigned ctos = hdr -> MissingBytes(position,numBytes);
    Allocate(ctos);
...
void OpenFile::Allocate(unsigned ctos){
    if (ctos > 0){
       Bitmap* freeMap = new Bitmap(NUM_SECTORS);
       OpenFile* freeMapFile;
       if (!fileSystem) //Formateando disco
         freeMapFile  = this;
       else 	   
	 freeMapFile  = fileSystem -> GetFreeMap();
       freeMap -> FetchFrom(freeMapFile);//Abrir mapa de sectores libres
       hdr -> Allocate(freeMap,ctos);
       freeMap -> WriteBack(freeMapFile);// Guardar mapa
    }
}



void OpenFile::Unlink(){
   LockWR* lockwr = openFileTable -> Get(sec);
   ASSERT(lockwr);
   lockwr -> Closer();
}
