...
#ifdef FILESYS
SynchDisk *synchDisk;
TableT<LockWR> *openFileTable;

//Función para concatenar strings
char* Concatenar(const char* str1,const char* str2){
  unsigned lenStr1 = strlen(str1), lenStr2 = strlen(str2);
  char* newStr = new char[lenStr1+lenStr2+1];
  strncpy(newStr,str1,lenStr1+1);
  strncat(newStr,str2,lenStr2+1);
  return newStr;
}

//Función para crear nuevas rutas
char* AbsolutePath(const char* dir, const char* fileName){
    unsigned lenDir = strlen(dir), lenFN = strlen(fileName);
    char* absPath = new char[lenDir+lenFN+2];
    if (!strcmp(dir,"")){
         strncpy(absPath,fileName,lenFN+1);
	 return absPath;
    }
    strncpy(absPath,dir,lenDir+1);
    strncat(absPath,"/",2);
    strncat(absPath,fileName,lenFN+1);
    return absPath;
}
...
Initialize(int argc, char **argv)
{
    ...
    // Inicialmente, el hilo main apunta el directorio root
    // Lo identificamos como ""
    ...
    paginationTable = new Bitmap(NUM_PHYS_PAGES);    
    SetExceptionHandlers();
#endif

#ifdef FILESYS
    #define MAX_CANT_OPEN_FILE 100
    synchDisk = new SynchDisk("DISK");
    openFileTable = new TableT<LockWR>(MAX_CANT_OPEN_FILE);
#endif
