...
Thread::Thread(const char *threadName, bool value, int prior,const char* path)
{
    p = nullptr;
    if(value){ // Si va a ver un join se necesita un puerto de comunicacion
      DEBUG('j',"Agregando un puerto\n");
      p = new Port();
    }
...
#endif
#ifdef FILESYS
    //Chequear que el directorio existe y 
    //almacenar una copia propia de la ruta    
    DEBUG('t',"Estoy en thread, soy %s\n",threadName);
    //El directorio root siempre existe
    unsigned lenDir = strlen(path);
    if (!strcmp(path,"")){        
	    myDir = new char[lenDir+1];
	    strncpy(myDir,"",1);
	    return;   
    }
    else{
    ASSERT(fileSystem -> DirectoryExist(path))
    myDir = new  char[lenDir+1];
    strncpy(myDir,path,lenDir);
    }
...
void Thread::SetPriority(int prior){
 ASSERT(priority >= 0 && priority < L_PRIORITY)
 priority = prior;
}
...
Thread::Fork(VoidFunctionPtr func, void *arg)
{
    ...
    //Se agrega al thread a la cola con prioridad priority
    scheduler->ReadyToRun(this,priority);
...
{
    DEBUG('u',"%p entra a finish\n",this);
    if (p != nullptr){ // Si se hace Join, existe un puerto
      DEBUG('j',"Enviando valor %d\n",value);
      p -> Send(value);
    }
    ...
    #ifdef USER_PROGRAM
    //Liberar páginas del thread
    unsigned numPages = machine -> GetMMU() -> pageTableSize;
...
int Thread::AddToTable(OpenFile* of){
  return fileTable -> Add(of);
}

OpenFile* Thread::GetOpenFile(int id){
  return fileTable -> Get(id);
}

void Thread::CloseOpenFile(int id){
  OpenFile* of = fileTable -> Remove(id);
  delete of;
}

char* Thread::GetMyDir(){
	return myDir;
}

void Thread::ChangeDirectory(const char* path){
#ifdef FILESYS	
    DEBUG('r',"cd td: %s\n",path); 
    DEBUG('r',"actual td: %s\n",myDir); 
    char* newDir;
    //Copiar ruta anterior si el directorio actual no es root
    if (strcmp(myDir,"") != 0){
	    newDir = AbsolutePath(myDir,path);
	    myDir = newDir;
    }
    else 
       //Concatenar ruta que se va a agregar
        newDir = Concatenar("",path);
     

    DEBUG('r',"newDir:%s\n",newDir);   
    if(!fileSystem -> DirectoryExist(newDir)){
        DEBUG('r',"Error: %s no existe\n",newDir);
	delete []  newDir;
    }
    else{	    
       myDir = newDir;       
       DEBUG('r',"Se cambio la ruta a %s\n",myDir);
    }
#endif    
}
