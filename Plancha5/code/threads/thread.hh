...
public:
    ...
    /// Initialize a `Thread`.
    Thread(const char *debugName,bool value,int prior,const char*);
    ...
    /// Bloquear hasta que un thread hijo termine
    void Join(int* j);

    //Agregar a la tabla de archivos abiertos
    int AddToTable(OpenFile* of);

    //Obtener el archivo identificado con id
    OpenFile* GetOpenFile(int id);

    //Cerrar el archivo identificado con id
    void CloseOpenFile(int id);
    ...
    //Cambiar el directorio actual del thread
