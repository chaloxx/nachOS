...
TimerInterruptHandler(void *dummy)
{
  ...
  // DEBUG('s',"Forzando otro cambio con ticks %u\n",stats -> totalTicks);
...
static void
MyTimerInterruptHandler(void *dummy)
{
   //DEBUG('s',"Forzando cambio con ticks %u\n",stats -> totalTicks);
   //Habilitar bandera
   interrupt->YieldOnReturn();   
  }
...
Initialize(int argc, char **argv)
{
    ...
    if (randomYield)            // Start the timer (if needed).
      ...
      timer = new Timer(MyTimerInterruptHandler, 0, randomYield);
    ...
    //Inicializar la paginationTable
