...
///
/// A hardware timer generates a CPU interrupt every X milliseconds.  This
/// means it can be used for implementing time-slicing.
///
...
///
/// Remember -- nothing in here is part of Nachos.  It is just an emulation
/// for the hardware that Nachos is running on top of.
///
...
///
/// * `timerHandler` is the interrupt handler for the timer device.  It is
///   called with interrupts disabled every time the the timer expires.
/// * `callArg` is the parameter to be passed to the interrupt handler.
/// * `doRandom` -- if true, arrange for the interrupts to occur at random,
...
Timer::TimeOfNextInterrupt()
{
    ...
    return TIMER_TICKS;//En caso de no tener cambios de contexto aleatoriamente
                           // fijamos una constante para que se produzcan cada TIMER_TICKS
}
