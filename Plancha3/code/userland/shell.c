...
main(void)
{
    ...
    for (;;) {
        ...
        //Debe el programa correrse en un segundo plano?
        if (line[0] == '&'){
          daemon = 1;
	...
	char* userland = "../userland/";
	unsigned lenU = length(userland), lenLine = length(line);
        char path[lenU+lenLine+1];
	cpy(path,userland,lenU+1);
	cat(path,line,lenLine+1);
        const SpaceId newProc = Exec(path,argv);

        // TO DO: check for errors when calling `Exec`; this depends on how
        //        errors are reported.
	// Si es un demonio correr en segundo plano
        if (!daemon)
          Join(newProc);
