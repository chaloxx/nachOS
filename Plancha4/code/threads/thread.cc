...
Thread::Thread(const char *threadName, bool value, int prior)
{
    p = nullptr;
    if(value){ // Si va a ver un join se necesita un puerto de comunicacion
      DEBUG('j',"Estoy en thread\n");
      p = new Port();
    }
    name     = threadName;
    stackTop = nullptr;
...
Thread::Finish(int value)
{
    ...
    //Si se usa la TLB esto no sirve
    ...
    for(unsigned i = 0; i < NUM_PHYS_PAGES;i++)
	    if (coremap[i].process == this){
	        coremap[i].process = nullptr;
		paginationTable -> Clear(i);
	    }  
    #endif
