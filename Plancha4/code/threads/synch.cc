...
Lock::Acquire()
{
  ASSERT(!IsHeldByCurrentThread());// Comprobar que el hilo actual no ah tomado el lock
  ...
  lockThread = currentThread; // Almacenar que hilo posee el mutex
}
...
Lock::Release()
{
  ...
  ASSERT(IsHeldByCurrentThread()); // Comprobar que se posee un mutex
  if(oldPriority != -1){
...
{
  return lockThread == currentThread;// Podemos identificar a cada hilo por su dirección de memoria
}
...
Condition::Condition(const char *debugName, Lock *conditionLock)
{
  ...
  name = debugName;
  l = conditionLock;// Asociar condición con lock
  queue = new List<Semaphore *>; // Necesitaremos una cola de semáforos
}
...
Condition::Wait()
{
  ...
  queue->Append(s); // Almacenamos el semáforo al final de la cola para conocer el orden de llegada de los hilos
